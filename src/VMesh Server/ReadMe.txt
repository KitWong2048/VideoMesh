================================================================================
    MFC 程式庫 : VMesh Server 專案概觀
===============================================================================

AppWizard 為您建立了 VMesh Server 應用程式。此應用程式不只展示
使用 MFC 的基礎用法，它也是個讓您學習如何撰寫應用程式的出發點。

這個檔案包含構成 VMesh Server 應用程式的所有檔案的摘要。
...
VMesh Server.h
    ...
    和宣告 CVMeshServerApp 應用程式類別。

VMesh Server.cpp
    這是包含應用程式類別 CVMeshServerApp 的主應用程式來源檔。
...
res\VMeshServer.ico
    這是個圖示檔，它被當成應用程式的圖示使用。主資源檔 VMesh Server.rc 
    中包含此圖示。
...
AppWizard 建立對話方塊類別:
VMesh ServerDlg.h, VMesh ServerDlg.cpp - 對話方塊
    這些檔案包含 CVMeshServerDlg 類別。此類別定義應用程式的主要對話方塊
...
StdAfx.h, StdAfx.cpp
    這些檔案用於建置名為 VMesh Server.pch 的先行編譯標頭檔 (PCH)，
