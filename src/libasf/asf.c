...
int
asf_init(asf_file_t *file, uint64_t *filepos)
{
	int tmp;
	if (!file)
		return ASF_ERROR_INTERNAL;

	tmp = asf_parse_header(file, filepos);
...
//	printf("packet: %d ", file->packet);
//	printf("position: %d ", file->position);

	packet->position = file->position;	// new added

	tmp = asf_data_get_packet(packet, file);

//	printf("length: %d ", packet->length);
//	printf("length: %d ", tmp);
//	printf("time: %d ", packet->send_time);
//	printf("duration: %d\n", packet->duration);
//	printf("tmp = %d\n",tmp);
	if (tmp < 0) {
		return tmp;
...
asf_seek_to_msec(asf_file_t *file, int64_t msec)
{
	...
	//printf("In Seek: file->data->packets_position = %d \t ", file->data->packets_position);
	//printf("file->packet_size = %d \n", file->packet_size);
