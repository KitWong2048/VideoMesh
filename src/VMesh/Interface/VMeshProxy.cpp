#include "../Interface/VMeshProxy.h"

using namespace std;

_VMESH_NAMESPACE_HEADER

VMeshProxy::VMeshProxy(string _myIP, int _myPort, string _remoteIP, int _remotePort, int _localStorageCapacity, int _proxyExchangeNeighbor)
: myVMesh(_myIP, _myPort, _localStorageCapacity, PROXY_IDENTITY, _proxyExchangeNeighbor)
{
...
VMeshProxy::~VMeshProxy()
{
	return;
}
	
bool VMeshProxy::start()
{
	currentTime = 0;
	return true;
}

void VMeshProxy::stop()
{
	myVMesh.stop();
	return;
}

bool VMeshProxy::movieJoin(const char* _key)
{
...
bool VMeshProxy::movieLeave(const char* _key)
{
	return true;
}

MediaInfo* VMeshProxy::getMediaInfo(const char* _key)
{
	return myVMesh.getMediaInfo();
}

void VMeshProxy::setTime(int _sec)
{
	myVMesh.clear_fetch();
	fetchedAID = 0;
	fetchedVID = 0;
...
Packet* VMeshProxy::getPacket(int _type)
{
	Packet* pac = NULL;
	if (_type==0)
	{
...
bool VMeshProxy::existPacket(int _type)
{
	if (_type==0)
