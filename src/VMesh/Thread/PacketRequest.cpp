...
void* PacketRequest::execute()
{
	...
	VMeshRemoteNodeList*popedNodes=new VMeshRemoteNodeList();
	int requestRetry=0;
	int getNodeRetry=0;
	int connectRetry=0;
	...
	while (keepRunning)
	{
		...
		for(requestRetry=0;requestRetry<3;requestRetry++)
		{
			...
			if(myNode==NULL)
			{
				...
				myNode=myExchangeTable->pop(PacketSeg);
			...
			{
				LOG_DEBUG("PR (" + itos((int)threadHandle%1000) +"***************Ping fails****************" + itos(PacketID) + "!");
		...
		*push back all node
		*/
		while(popedNodes->getSize()>0)
		{
			myExchangeTable->push(popedNodes->getNode(0));
			popedNodes->sharrowRemoveNode(popedNodes->getNode(0));
		}
