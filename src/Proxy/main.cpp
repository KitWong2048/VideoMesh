...
#include <fstream>
#include <iostream>
#include "..\Proxy\Common\Common.h"
#include "..\VMesh\Interface\VMeshProxy.h"
using std::string;
...
void UpdateLogServerInfo( void *data );
...
CRITICAL_SECTION vmproxy_criticalsection;
int command;
int	port;
int thread_count;
string hash;
string server;
int proxyExchangeNeighbor;
...
// For getting the playlist to store at current directory
...
void readConfigFile()
{
	...
	while(option != NULL)
	{
		...
		else if (strcmp(option, "ProxyExchangeNeighbor") == 0)
		{
			value = strtok(NULL, "\r\n");
			if (value != NULL)
				proxyExchangeNeighbor = atoi(value);
		}
	...
	cout<<"ProxyExchangeNeighbor is"<<proxyExchangeNeighbor<<endl;
	/*cout<<"TotalBandwidth is"<<totalbandwidth<<endl;
	cout<<"ServerConsumeBandwidth is"<<serverbandwidth<<endl;
	cout<<"ProxyConsumeBandwidth is"<<proxybandwidth<<endl;
	cout<<"ClientConsumeBandwidth is"<<clientbandwidth<<endl;*/
...
int main(int argc, char *argv[])
{
	...
	std::stringstream ss(argv[1+startingParameter]);
	ss >> port;

	std::stringstream ss2(argv[2+startingParameter]);
	ss2 >> listen_port;

	server = argv[3+startingParameter];*/

	/////////////////////// Getting log server info from central lookup server ///////////////////////
	/////////////////////// End of getting log server info  //////////////////////////////////////////////
	char strTemp[100];
	std::stringstream ss(listen_port);
	string string_temp = ss.str();

	//hUpdateLogServerInfoThread = (HANDLE)_beginthread(UpdateLogServerInfo , 0,(void *)&server);
	...
	InitializeCriticalSection (&vmproxy_criticalsection);
	...
	if ( server_socket )
	{
		if (  argc == ( 5 + startingParameter ) )
		{
			int numOfSegments;
			std::stringstream ss(argv[4+startingParameter]);
			ss >> numOfSegments;
			status = WaitForClientConnections(server_socket, numOfSegments);
		}
		else
			status = WaitForClientConnections(server_socket);
	...
	DeleteCriticalSection (&vmproxy_criticalsection);
...
int WaitForClientConnections(SOCKET server_socket, int num_of_segments)
{
	...
	do
	{
		...
		cout << "Check the External IP before entering into UPnP::\t" << szExternalIPAddress << endl;
		...
		if ( bTryAlternativelyMethod )
		{
			...
			URLDownloadToCacheFile(NULL, url, pathToFile, sizeof(pathToFile), 0, 0);
			FILE* cacheFile;
		...
		cout << "Check the External IP after entering into UPnP::\t" << szExternalIPAddress << endl;
		//VMeshProxy vmeshProxy(szExternalIPAddress, available_port, server, port, num_of_segments);		
		
		//VMeshProxy vmeshProxy(szExternalIPAddress, available_port, server, port, num_of_segments, 2);
		VMeshProxy vmeshProxy(szExternalIPAddress, available_port, server, port, num_of_segments, proxyExchangeNeighbor);
		vmProxy = &vmeshProxy;
		...
		if (vmProxy->movieJoin(hash.c_str()))
			SetEvent(hReady);
		else
		{
			vmProxy->stop();
		...
		for ( ;; )
		{
			...
			m_Thread = (HANDLE)_beginthread(HandleHTTPRequest,0,(void *)ci);
		}
...
void HandleHTTPRequest( void *data )
{
	...
	{	
		...
		if ( strstr(requestheader.method,"GET") )
		{
			...
			sprintf(msg, "Content-Length: %d\r\n", vmProxy->getMediaInfo(hash.c_str())->getFileSize());
...
//////////////////////////////////////////////////////////////////////////////////////////
			...
			if (vmProxy->getMediaInfo(hash.c_str())->getMediaLength(AUDIO_STREAM)/1000 <= settime)
			{
				ThreadLog(thread_no, "Set time error\n");
				break;
			}
			
			EnterCriticalSection (&vmproxy_criticalsection);
			...
			vmProxy->setTime(settime);

			char* appData;
			int appLen = vmProxy->getMediaInfo(hash.c_str())->getAppData((const char*&)appData);
			appLen = appLen - 8;
			time(&curTime);
			...
			if ( -1 == SocketWrite(client_socket , appData +8 , appLen) )
			{
				...
				if ( retry >= 5 )
				{
					...
					closesocket(client_socket);
					LeaveCriticalSection (&vmproxy_criticalsection);
					break;
			//	}
			...
			MediaInfo* info = vmProxy->getMediaInfo(hash.c_str());
			...
			/*while (1)
			{
				...
				int cnt = 0;
				if (!vmProxy->existPacket(AUDIO_STREAM))
					if (vmProxy->getMediaInfo(hash.c_str())->getIntervalbyPID(pid + 1, AUDIO_STREAM) == -1)
					{
				...
				while ((++cnt < 300) && !vmProxy->existPacket( AUDIO_STREAM))
					Sleep(100);

				if (!vmProxy->existPacket(AUDIO_STREAM))
				{		
					char msg[100];
					sprintf(msg, "No packet %d (time: %ds), waited 30s \n", packetSeq, vmProxy->getMediaInfo(0)->getIntervalbyPID(packetSeq, 0));
					ThreadLog(thread_no, msg);
				...
				//start = GetTickCount();
				pkt = vmProxy->getPacket(AUDIO_STREAM);
				if (pkt==NULL)
				{
				...
				if ( packetSeq != pkt->getPacketID() )
				{
					sprintf(msg, "Missing packet %u (time: %ds) \t The packet received is %u \n", packetSeq
						, vmProxy->getMediaInfo(0)->getIntervalbyPID(packetSeq, 0), pkt->getPacketID() );
					ThreadLog(thread_no, msg );
					packetSeq = pkt->getPacketID();
...
//--------------------------------------------------------------
//	getPlaylist()
//		get playlist from the VMesh Server
//--------------------------------------------------------------
void getPlaylist()
{
